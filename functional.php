<!DOCTYPE html>
<?php include('include/html-head.php'); ?>
    <div class="container">
        <div class="articletitle">
            <div class="container">
                <h1>{{funkcionalis_cim1}}</h1>
                <h2>{{funkcionalis_cim2}}</h2>
                <div class="ulholder">
                    <p class="intro">{{funkcionalis_cim3}}</p>
                    <ul>
                        <li> {{funkcionalis_cim4}} </li>
                        <li> {{funkcionalis_cim5}} </li>
                        <li> {{funkcionalis_cim6}} </li>
                        <li> {{funkcionalis_cim7}} </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="articlecover_funkc"></div>
        <div class="articlebody_g">
            <div class="container">
                <h6>{{funkcionalis_leiras1}} </h6>
                <p> {{funkcionalis_leiras2}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_leiras3}}<br></h6>
                <p> {{funkcionalis_leiras4}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_leiras5}}<br></h6>
                <p> {{funkcionalis_leiras6}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_leiras7}}<br></h6>
                <p> {{funkcionalis_leiras8}}
                    <br> </p>
            </div>
        </div>
        <div class="articlebody_w">
            <div class="container">
                <h2>{{funkcionalis_folyamat1}}<br><br></h2>
                <p> {{funkcionalis_folyamat2}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_folyamat3}}<br></h6>
                <p> {{funkcionalis_folyamat4}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_folyamat5}}</h6>
                <p> {{funkcionalis_folyamat6}}
                    <br>
                    <br> </p>
                <h6>{{funkcionalis_folyamat7}}</h6>
                <p> {{funkcionalis_folyamat8}} </p>
            </div>
        </div>
    </div>
    <?php include('include/bottom-components.php'); ?>
        </body>

        </html>